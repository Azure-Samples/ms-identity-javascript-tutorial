...
urlFragment: ms-identity-javascript-signin
description: "This sample demonstrates a Vanilla JavaScript single-page application (SPA) that lets users sign-in with Azure AD"
---

| In this Tutorial |Previous Tutorial | Next Tutorial | All Content |
|------------------|------------------|----------------|------------|
| authentication, id tokens, single sign-on |  | [Acquire an Access Token and call MS Graph API](https://github.com/Azure-Samples/ms-identity-javascript-v2) | [Table of Contents](https://github.com/Azure-Samples/ms-identity-javascript-tutorial) |
...
```console
git clone https://github.com/Azure-Samples/ms-identity-javascript-signin.git
```
...
```console
    cd ms-identity-javascript-signin
    npm install
```
...
#### Register the app (ms-identity-javascript-signin)

1. Navigate to the Microsoft identity platform for developers [App registrations](https://go.microsoft.com/fwlink/?linkid=2083908) page.
1. Select **New registration**.
1. In the **Register an application page** that appears, enter your application's registration information:
   - In the **Name** section, enter a meaningful application name that will be displayed to users of the app, for example `ms-identity-javascript-signin`.
...
#### Configure the app (ms-identity-javascript-signin) to use your app registration
...
1. Find the app key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of the `ms-identity-javascript-signin` application copied from the Azure portal.
1. Find the app key `Enter_the_Cloud_Instance_Id_Here/Enter_the_Tenant_Info_Here` and replace the existing value with `https://login.microsoftonline.com/<your-tenant-id>`.
1. Find the app key `Enter_the_Redirect_Uri_Here` and replace the existing value with the base address of the ms-identity-javascript-signin project (by default `http://localhost:3000`).
...
> :information_source: Consider taking a moment to [share your experience with us](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbR73pcsbpbxNJuZCMKN0lURpUQkxOQTVPTFNSMFVZQUJMVVUwMkNBOElNWSQlQCN0PWcu).
...
#### Build and upload the `ms-identity-javascript-signin` to an Azure Storage blob
...
1. In the `ms-identity-javascript-signin` project source code, update your configuration file with the **Primary endpoint field** as your new **Redirect URI** (you will register this URI later).
...
##### Update the Azure AD app registration for `ms-identity-javascript-signin`

1. Navigate back to to the [Azure portal](https://portal.azure.com).
1. In the left-hand navigation pane, select the **Azure Active Directory** service, and then select **App registrations**.
1. In the resulting screen, select the `ms-identity-javascript-signin` application.
1. From the *Branding* menu, update the **Home page URL**, to the address of your service, for example [https://ms-identity-javascript-signin-contoso.azurewebsites.net](https://ms-identity-javascript-signin-contoso.azurewebsites.net). Save the configuration.
