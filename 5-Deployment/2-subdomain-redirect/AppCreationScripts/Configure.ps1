...
#> 
   ...
   Write-Host "Creating the AAD application (DomainTestSPA)"
   # create the application 
   $spaAadApplication = New-AzureADApplication -DisplayName "DomainTestSPA" `
                                               -HomePage "https://example.com:3000" `
                                               -ReplyUrls "https://example.com:3000" `
                                               -IdentifierUris "https://$tenantName/DomainTestSPA" `
                                               -PublicClient $False
   ...
   Write-Host "Done creating the spa application (DomainTestSPA)"
   ...
   Add-Content -Value "<tr><td>spa</td><td>$currentAppId</td><td><a href='$spaPortalUrl'>DomainTestSPA</a></td></tr>" -Path createdApps.html


   # Update config file for 'spa'
   $configFile = $pwd.Path + "\..\SPA1\App\authConfig.js"
   ...
   # Update config file for 'spa'
   $configFile = $pwd.Path + "\..\SPA2\App\authConfig.js"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "Enter_the_Application_Id_Here" = $spaAadApplication.AppId;"Enter_the_Tenant_Info_Here" = $tenantId;"Enter_the_Redirect_Uri_Here" = $spaAadApplication.ReplyUrls };
   ReplaceInTextFile -configFilePath $configFile -dictionary $dictionary
   Write-Host ""
